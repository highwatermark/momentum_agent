...
def update_candidate_outcomes(data_client):
    ...
    # Get candidates that need outcome updates (1-10 days old, missing outcomes)
    cursor.execute("""
    ...
    for candidate in candidates:
        ...
        try:
            # Fetch current/recent price for this symbol
            from alpaca.data.requests import StockSnapshotRequest
            request = StockSnapshotRequest(symbol_or_symbols=[symbol])
            snapshot = data_client.get_stock_snapshot(request)

            if symbol in snapshot and snapshot[symbol].daily_bar:
                current_price = snapshot[symbol].daily_bar.close
                days_elapsed = (datetime.now() - entry_time).days

                # Update appropriate price field
                if days_elapsed >= 1 and candidate['price_1d_later'] is None:
                    cursor.execute(
                ...
                if days_elapsed >= 10:
                    ...
                    would_have_won = 1 if (current_price - entry_price) / entry_price > 0.03 else 0
                    cursor.execute(
            ...
            print(f"Error updating outcome for {symbol}: {e}")
            continue
...
def calculate_reward(trade: dict) -> float:
    ...
    max_dd = trade.get('max_drawdown_during_trade', 0.05)

    # Reward components
    hold_penalty = -0.001 * holding_days  # Penalize long holds
    risk_adjusted = pnl_pct / max_dd if max_dd > 0 else pnl_pct
